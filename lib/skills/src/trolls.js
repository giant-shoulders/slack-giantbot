...
const trollEm = (bot, message, user) => {
  return function() {
    let trollage = arrayRandom(TROLLS).format({ user });
...
const setTimer = (bot, message, user, timeout) => {
  ...
  let userTimer = arrayFind(timers, 'user', user);
  if (userTimer) { return; }
...
const skill = ({ controller }) => {
  ...
  controller.on('message_received', (bot, message) => {
    ...
    if (!typistEntry) {
      typistEntry = { user: user, delay: timeout, count: 1 };
