...
const skill = ({ controller }) => {
  controller.hears(trigger, 'ambient', (bot, message) => {
    ...
    const botChoice = choices[diceRoll({ sides: choices.length })[0] - 1];
    const result = compare(choiceMap, botChoice, userChoice);
    const resultString = (result === 'TIE') ? 'TIE' : (result === 'BOT') ? 'LOSS' : 'WIN';

    bot.reply(message, `
*${resultString}* - *${userChoice.toUpperCase()} (PLAYER)* _vs_ *${botChoice.toUpperCase()} (BOT)*
