...
const sendPoll = (feed, article, bot) => (
  () => {
    bot.say({
      text: `*New Atom/RSS Article from ${changeCase.title(feed.id)}:*\n${article.title}\n${article.link}`,
...
const updateMostRecent = (controller, feed, article) => {
  controller.storage.channels.get(feed.channelId, (getErr, data = {}) => {
    ...
    currentFeed.mostRecent = article.link;
    feeds[currentFeedIndex] = currentFeed;

    controller.storage.channels.save({ id: feed.channelId, feeds });
  });
};

// Polling feeds
const pollAllFeeds = (feeds, controller, bot) => {
  let delay = 0;
  ...
  feeds.forEach(feed => {
    feedReader(feed.url, (err, articles) => {
      if (err) {
        bot.say({
          text: `Something went wrong getting the feed for *${feed.id}* :face_with_head_bandage:`,
          channel: feed.channelId,
        });
      }
      ...
      // Get a list of unread articles based on most recent read url
      if (feed.mostRecent) {
        let i = 0;
        let article = articles[i];

        while (article && article.link !== feed.mostRecent) {
      ...
      // Trim to 5 most recent to prevent spam
      unreadArticles = unreadArticles.splice(0, 5);

      // Save Most Recent Link
      if (unreadArticles.length) {
        // Update Most Recent as first item
        updateMostRecent(controller, feed, unreadArticles[0]);
  ...
  // Set our timer
  timer = setTimeout(() => {
    pollAllFeeds(feeds, controller, bot);
  }, POLL_DELAY);
};
...
const skill = ({ controller }) => {
  ...
  controller.middleware.receive.use((bot, message, next) => {
    if (!running) {
      running = true;
      getAllFeeds(controller).then(feeds => pollAllFeeds(feeds, controller, bot));
    }
  ...
  controller.hears(triggerListFeed, sources, (bot, message) => {
    const listFeeds = (response) => {
      ...
      bot.reply(message, {
        attachments: [{
          ...
          text: `Here's all the atom/rss feeds that I'm aware of so far, you may have to click 'Show more...' to view them all:\n${formattedFeeds.join('\n')}`,
          color: '#439FE0',
          mrkdwn_in: ['text'],
        }],
      });
    ...
    getAllFeeds(controller)
      .then(listFeeds)
      .catch(() => bot.reply(message, `Something went wrong getting the feeds. :face_with_head_bandage:`));
  });
};
